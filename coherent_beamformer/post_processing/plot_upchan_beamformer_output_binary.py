...
# 1k mode
#N_coarse = 1
#N_fine = N_coarse*(4096*1024)/8 # N_coarse*2^19
# 4k mode
N_coarse = 4 # 4
N_fine = N_coarse*(1024*1024)/8 # N_coarse*2^17 
# 32k mode
#N_coarse = 32
#N_fine = N_coarse*(128*1024)/8 # N_coarse*2^14

# Reshape array to 3D -> Time X Bins X Beams
contents_array = contents_float[0:(N_time*N_fine*N_beam)].reshape(N_beam,N_time,N_fine)
...
plt.imshow(contents_array[beam_idx,0:N_time,0:N_fine], extent=[1, N_fine, 1, N_time], aspect='auto', interpolation='none')
...
plt.plot(contents_array[beam_idx,time_idx,0:N_fine])
...
axs[0, 0].plot(contents_array[0,time_idx,0:N_fine])
axs[0, 0].set_title('Beam 1')
axs[0, 1].plot(contents_array[1,time_idx,0:N_fine], 'tab:orange')
axs[0, 1].set_title('Beam 2')
axs[1, 0].plot(contents_array[2,time_idx,0:N_fine], 'tab:green')
axs[1, 0].set_title('Beam 3')
axs[1, 1].plot(contents_array[3,time_idx,0:N_fine], 'tab:red')

...
contents_array = contents_float[0:(N_coarse*N_win*N_pol*N_ant*N_fine*N_iq)].reshape(N_win,N_fine,N_coarse,N_pol,N_ant,N_iq)
    ...
    for w in range(0,N_win):
        for p in range(0,N_pol):
            for a in range(0,N_ant):
                ...
                X[w,p,a,(0+c*N_fine):(N_fine+c*N_fine)] = np.sqrt(np.square(contents_array[w,0:N_fine,c,p,a,0]) + np.square(contents_array[w,0:N_fine,c,p,a,1]))

                for iq in range(0,N_iq):
                    # FFT output (amplitude)
                    contents_restructure[w,p,a,(0+c*N_fine):(N_fine+c*N_fine),iq] = contents_array[w,0:N_fine,c,p,a,iq]
